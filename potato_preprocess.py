...
def main():
    ...
    Y_NG = np.ones(int(len(X_NG)/pix))#make teach data
    X_NO = Preprocess(NoP_L)
    Y_NO = np.full(int(len(X_NO)/pix),2)#make teach data
...
def TakePics():
    ...
    for i in range(20):
        print(i)
        camera.capture('//home/pi/ドキュメント/potato_classfier/3_NoPotato/%s.jpg' % i)
